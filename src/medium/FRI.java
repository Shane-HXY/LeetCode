 ...
 * }
 */
public class FRI {  //  Find Right Interval
    ...
    public static int[] findRightInterval(Interval[] intervals) {
        ...
        for (int i = 0; i < intervals.length; i++) {
            Map.Entry<Integer, Integer> out = map.ceilingEntry(intervals[i].end);
            if (out != null) {
                res[i] = out.getValue();
            } else {
                res[i] = -1;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.asList(findRightInterval(intervals)));
        for (int i = 0; i < intervals.length; i++) {
            System.out.println(findRightInterval(intervals)[i]);
        }
    }
}
